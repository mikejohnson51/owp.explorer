...
ui <- bootstrapPage(tags$style(type = "text/css", "html, body, .leaflet {width:100%; height:100%}"),
                    ...
                    absolutePanel(top = 10, left = 100, draggable = TRUE,
...
server <- function(input, output, session) {
    ...
    version = "CAMELS20"
    xx = as.vector(sapply(get_bucket(bucket = bucket, prefix = version), "[[", 1))
    ...
    if(input$path != ""){
        ...
        shiny::updateSelectInput(session, "att", choices = c[-1], selected = "")

        cats <<- st_transform(s3read_using(read_sf, object = p, "catchments"), 4326)
        fps  <<- st_transform(s3read_using(read_sf, object = p,   "flowpaths"), 4326)
        nex  <<- st_transform(s3read_using(read_sf, object = p,   "nexus"), 4326)
        bb = sf::st_bbox(cats)

        leaflet::leafletProxy("map") |>
            clearShapes() |>
            addPolygons(data = cats, fillColor = 'gray', color = "black") |>
            addPolylines(data = fps, weight = 1, opacity = 1) |>
            addCircles(data = nex, radius = 100, fillOpacity = 1, fillColor = "red", stroke = FALSE) |>
            fitBounds(lat1 = as.numeric(bb$ymin), lat2 = as.numeric(bb$ymax),
                      lng1 = as.numeric(bb$xmin), lng2 = as.numeric(bb$xmax))
    }
})
    ...
    observeEvent(input$att, {
        ...
        leaflet::leafletProxy("map") |>
            ...
            addPolylines(data = fps, weight = 1, opacity = 1) |>
            addCircles(data = nex, radius = 100, fillOpacity = 1, fillColor = "red", stroke = FALSE) |>
            addLegend(position = "bottomleft", pal = qpal, values = v, opacity = 1, title = selection)
        }
    })
